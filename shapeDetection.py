    ...
    for cnt in contours:
        ...
        if area > 500:
            ...
            if objCor == 3:
                ...
                aspRatio = w/float(h)
                if aspRatio > 0.95 and aspRatio < 1.05: # deviation of 5 percent
                    objectType = 'Square'
                else:
...
path = 'shapes/shapes.jpg'
img = cv2.resize(cv2.imread(path), (800, 400))
imgContour = img.copy()

imgGray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
imgBlur = cv2.GaussianBlur(imgGray, (7,7), 1)
imgCanny = cv2.Canny(imgBlur, 50, 50) # find edges
